...
2. `package.json`, lockfile, `.nvmrc`, 설정 파일, docs, scripts를 확인합니다.
3. 기존 `README.md`가 있으면 구조를 가져오지 말고, 프로젝트 역할, 실행 방법, 생성 파일, 수정 기준 같은 확인된 사실만 추출합니다.
4. README 원본은 반드시 `PRESET.md`의 구성에 대상 레포지토리의 확인된 사실을 채워 넣어 `resources/README.preset.md`로 새로 작성합니다.
5. README 자동 재생성이 필요하면 생성 스크립트나 명령을 단순하고 재현 가능하게 작성하며, 대상 레포지토리 형태에 따라 분기할 수 있게 둡니다.
6. 대상 레포지토리에 `package.json` 같은 script manifest가 있고 README 생성 명령을 만들었다면 해당 scripts 목록에 포함합니다.
7. `README.md` 상단에는 생성 파일임을 알리는 주석을 둡니다.
...
- badge는 대상 레포지토리의 실제 기술 스택, 패키지 매니저, 배포 상태, 버전 정보가 확인되는 경우에만 추가합니다.
...
대상 레포지토리의 `resources/README.preset.md`는 이 저장소의 README를 그대로 복사하지 않고, [`PRESET.md`](./PRESET.md)를 참고해 대상 레포지토리에 맞게 재작성합니다.

`PRESET.md`는 에이전트가 읽는 참조용 파일입니다. 실제 적용 시에는 대상 레포지토리 안에 `resources/README.preset.md` 파일을 생성합니다.

위 템플릿은 기본 골격입니다. 대상 레포지토리에 맞지 않는 섹션은 삭제하고, 필요한 섹션은 추가합니다. `README 수정 방법`은 주요 H2 섹션으로 두지 않고, 생성 파일 주석과 대상 `AGENTS.md`의 README 수정 지침에 둡니다.
...
- Hero/Title: 프로젝트 이름, 한 줄 설명, 버전 또는 상태 badge
