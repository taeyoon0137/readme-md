...
- secret, token, credential은 저장소에 기록하지 않습니다.
...
`CLAUDE.md`는 `AGENTS.md`를 가리키는 심볼릭 링크로 둡니다.
...
3. symlink을 끝내 사용할 수 없는 환경이라면 `CLAUDE.md`를 `AGENTS.md`의 일반 파일 사본으로 둡니다. 이 경우 두 파일이 어긋나지 않도록 다음을 함께 명시합니다.
...
- README의 한 줄 설명(`<p align="center">` 안 description)도 대상 `resources/README.preset.md`에 최종 문구를 직접 작성합니다. `${description}` 같은 placeholder를 남겨두지 않으며, 생성 명령이나 스크립트가 description을 생성하거나 덮어쓰지 않습니다.
- README의 히어로 이미지 `alt` 텍스트도 대상 `resources/README.preset.md`에 최종 문구를 직접 작성합니다. `${projectName} hero image` 같은 placeholder를 남겨두지 않으며, 생성 명령이나 스크립트가 alt 텍스트를 생성하거나 덮어쓰지 않습니다.
...
- `resources/readme-hero.preset.svg`의 `<image>` 순서상 `hero.light.*`, `hero.dark.*`는 각 모드에서 `hero.png` 또는 `hero.jpg`를 덮어 그립니다. 즉 라이트/다크 이미지가 같은 모드에서 우선 표시됩니다. 모드별 이미지를 추가했는데 다른 모드에는 기본 이미지를 그대로 두고 싶다면 `hero.png` 또는 `hero.jpg`를 함께 유지합니다.
...
- README의 한 줄 설명과 히어로 `alt` 텍스트는 `resources/README.preset.md`에 최종 문구를 직접 작성하고 placeholder를 남기지 않습니다.
- 다크/라이트 모드 전용 히어로가 필요하면 `resources/hero.dark.png`, `resources/hero.dark.jpg`, `resources/hero.light.png`, `resources/hero.light.jpg`를 추가합니다. 같은 모드에서 `hero.png`/`hero.jpg`가 같이 존재하면 라이트/다크 이미지가 위에 덮입니다.
...
- `CLAUDE.md` 관련 작업을 했다면 `CLAUDE.md`가 `AGENTS.md`를 가리키는 심볼릭 링크인지 확인합니다. symlink을 사용할 수 없는 환경에서는 일반 파일 사본의 `diff AGENTS.md CLAUDE.md` 결과가 비어 있는지 확인합니다.
- whitespace 오류나 conflict marker가 남아 있지 않은지 `git diff --check`로 확인합니다.
- 변경 범위는 `git status --short`로 한 번 더 확인합니다.
